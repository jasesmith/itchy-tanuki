(function($angular, _, Hammer) {
  ...
  .directive('colorPan', ['UiHelpers', function(ui){
    return {
      controller: ['$scope', 'ColorService', 'UiHelpers', function($scope, colors, ui){
        ...
        .on('pan panmove', function panmove(e) { // _.throttle(, 30)
          ...
          // BAHABAHABAHABAAHBAGHG!!!!

          k = ui.getColorByDegree($scope.shades, l, rotationDeg, n);
          $scope.tuner = k.tuner;
          $scope.color = ui.setColor(k.color, $scope.color, false, true);
          lastDeg = rotationDeg;
          $scope.$apply();
        })
  ...
  .controller('app', ['$scope', '$document', 'LocalStorage', 'ColorService', 'UiHelpers', function($scope, $document, storage, colors, ui) {
    ...
    var hotkeys = function hotkeys(e){
      ...
      if(e.which === 27){ // esc
        $scope.preferences.showMenu = false;
        $scope.preferences.showToolbox = false;
    ...
    $scope.doIt = function doIt(color){
      ...
      $scope.color = ui.setColor(color, $scope.color, $scope.preferences.useHash);
    ...
    $scope.setStateClases = function setStateClases(prefs){
      if(prefs){
        var classes = [];
        if(prefs.showFavs){
          classes.push('favorites-active');
        }
        if(prefs.fancy){
          classes.push('fancy');
        }
        if(prefs.alwaysShowHex){
          classes.push('always-show-hex');
        }
        if(prefs.showMenu){
          classes.push('menu-active');
        }
        if(prefs.showToolbox){
          classes.push('toolbox-active');
        }
        if(prefs.showHistory){
          classes.push('history-active');
        }
        return classes.join(' ');
      }
    };
    ...
    $scope.setClasses = function setClasses(color, test, affectText){
      if(color) {
        var classes = [];
        if(affectText) {
          classes.push(colors.brightness(color) < 155 ? 'fg-light' : 'fg-dark');
        }
        if(color === test) {
    ...
    $scope.$watch('preferences', function (n, o) {
      if(n && o && n !== o) {
        ...
        if((n.useHash !== o.useHash)) {
            $scope.color = ui.setColor($scope.color, $scope.color, n.useHash, !n.useHash);
        }
      }
    }, true);
